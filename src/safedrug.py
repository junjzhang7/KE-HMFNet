...
from safedrug_utils import llprint, multi_label_metric, ddi_rate_score, get_n_params, buildMPNN
...
def main():
    ...
    #         output, _ = model(test_sample[:test_adm_idx+1])
    #         y_gt_tmp = np.zeros(voc_size[2])
    #         y_gt_tmp[adm[2]] = 1
    ...
    #     fig.savefig('safedrug_effect_of_visit_len_for_various_models.pdf')
    #     return
    if args.casestudy:
        model.load_state_dict(
            torch.load(open(args.resume_path, "rb"), map_location=device))
        ...
        for step, input in enumerate(data_train):
            ...
            for idx, adm in enumerate(input):
                ...
                current_ddi_rate = ddi_rate_score(
                    [[y_label]],
                    path="/home/zjj/code/MoleRec_cus/data/ddi_A_final.pkl")
                ...
                if current_ddi_rate <= args.target_ddi:
                    ...
                    beta = max(
                        0, 1 - (current_ddi_rate - args.target_ddi) / args.kp)
